...
\begin{abstract}
This paper presents a highly optimized FPGA implementation of the Advanced Encryption Standard (AES-128) cipher targeting resource-constrained embedded systems. The design incorporates three key innovations: on-the-fly key expansion reducing storage by 81.8\%, decomposition matrix optimization for MixColumns resource sharing achieving 34.9\% area savings, and dual S-box architecture for differential power analysis (DPA) resistance. Implemented on the Xilinx Artix-7 XC7A100T FPGA, the design achieves 100 MHz operation with only 2,132 LUTs (3.36\% utilization), 2,043 registers (1.61\%), zero Block RAM usage, 172 mW total power consumption (1.72 nJ/bit energy efficiency), and 100-152 Mbps throughput. Comprehensive verification using NIST FIPS-197 test vectors demonstrates 100\% functional correctness. The balanced optimization across area, power, and performance makes this implementation ideal for IoT devices, secure communications, and low-power embedded security applications.
\end{abstract}
...
Power (mW) & 245 & 856 & 156 & 89 & 47 & 178 & \textbf{172} \\ \hline
Energy (nJ/bit) & 2.89 & 5.35 & 3.11 & 2.31 & 1.83 & 3.15 & \textbf{1.72} \\ \hline
...
\begin{enumerate}
    ...
    \item \textbf{Competitive power consumption (172 mW total):}
    \begin{itemize}
        \item Sequential processing reduces simultaneous switching activity
        \item Zero BRAM avoids static RAM leakage current
        \item Optimized critical path enables lower voltage operation potential
        \item Lower than high-speed implementations [1,2,6] by 1.0-5.0$\times$
    \end{itemize}

    \item \textbf{Excellent energy efficiency (1.72 nJ/bit):}
    \begin{itemize}
        \item Third-best among all implementations
        \item Only Patel et al. [5] (1.83 nJ/bit) and Hassan et al. [4] (2.31 nJ/bit) achieve comparable efficiency
        \item Significantly better than high-speed designs [1,2] at 2.89-5.35 nJ/bit
    \end{itemize}
...
Total Power (Dynamic + Static) & 172 mW \\ \hline
...
\textbf{Energy per Bit (Encryption)} & \textbf{1.72 nJ/bit} \\ \hline
Energy per Bit (Decryption) & 2.39 nJ/bit \\ \hline
Energy per Block (128-bit Enc.) & 220 nJ \\ \hline
Energy per Block (128-bit Dec.) & 306 nJ \\ \hline
...
\begin{equation}
Energy/bit = \frac{Total\ Power \times Time\ per\ Block}{Block\ Size} = \frac{Total\ Power}{Throughput}
\end{equation}
\begin{equation}
Energy/bit_{enc} = \frac{0.172\ W \times 1.28\ \mu s}{128\ bits} = \frac{0.172\ W}{100\ Mbps} = 1.72\ nJ/bit
\end{equation}
...
\textbf{Energy Efficiency (Enc.)} & \textbf{1.72 nJ/bit} \\ \hline
Energy Efficiency (Dec.) & 2.39 nJ/bit \\ \hline
...
\begin{enumerate}
    ...
    \item \textbf{Power Efficiency:} Demonstrated 172 mW total power consumption with 1.72 nJ/bit energy efficiency, making the design suitable for battery-operated IoT devices and energy-constrained embedded systems.
